...
public class IrisEntityVertex {
	...
	/** Same as above, with the Iris entity ids passed in (captured on the render thread; for worker threads). */
	public static void write(long ptr, float x, float y, float z, int color, float u, float v, float mid_u, float mid_v, int overlay, int light, int normal, int tangent,
							 short entity, short blockEntity, short item) {
		PositionAttribute.put(ptr + 0L, x, y, z);
		ColorAttribute.set(ptr + 12L, color);
		TextureAttribute.put(ptr + 16L, u, v);
		OverlayAttribute.set(ptr + 24L, overlay);
		LightAttribute.set(ptr + 28L, light);
		NormalAttribute.set(ptr + 32L, normal);
		MemoryUtil.memPutShort(ptr + ENTITY_ID_OFFSET, entity);
		MemoryUtil.memPutShort(ptr + BLOCK_ENTITY_ID_OFFSET, blockEntity);
		MemoryUtil.memPutShort(ptr + ITEM_ID_OFFSET, item);
		MemoryUtil.memPutFloat(ptr + MID_U_OFFSET, mid_u);
		MemoryUtil.memPutFloat(ptr + MID_V_OFFSET, mid_v);
		MemoryUtil.memPutInt(ptr + TANGENT_OFFSET, tangent);
	}

	/** Render thread: copies Iris' "currently rendered" ids into the draw snapshot. */
	static void captureIds(DrawParams p) {
		p.entityId = (short) CapturedRenderingState.INSTANCE.getCurrentRenderedEntity();
		p.blockEntityId = (short) CapturedRenderingState.INSTANCE.getCurrentRenderedBlockEntity();
		p.itemId = (short) CapturedRenderingState.INSTANCE.getCurrentRenderedItem();
	}
